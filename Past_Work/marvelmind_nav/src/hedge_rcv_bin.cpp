...
nav_msgs::Odometry hedge_pos_noaddress_msg;// hedge coordinates message (old version without address) for publishing to ROS topic
...
static int hedgeReceivePrepare(int argc, char **argv)
{
    ...
    if (argc==2) ttyFileName="/dev/marvelmindips";
    else ttyFileName="/dev/marvelmindips";
  ...
  if (hedge->haveNewValues_)
    {
        ...
        if (hedge_pos_msg.flags&(1<<1))// flag of timestamp format 
          {
			...
			hedge_pos_noaddress_msg.header.stamp = ros::Time::now();
		  }	
	     else 
	      {
        ...
        hedge_pos_noaddress_msg.pose.pose.position.x = (int(position.x/1000.0)/10); 
        hedge_pos_noaddress_msg.pose.pose.position.y = (int(position.y/1000.0)/10); 
        hedge_pos_noaddress_msg.pose.pose.position.z = (int(position.z/1000.0)/10);
        hedge_pos_noaddress_msg.header.frame_id = "base_ips_link";
        hedge_pos_noaddress_msg.child_frame_id = "base_link";
...
{uint8_t beaconReadIterations;
  ...
  ros::Publisher hedge_pos_noaddress_publisher = n.advertise<nav_msgs::Odometry>(HEDGE_POSITION_TOPIC_NAME, 1000);
  ...
  hedge_pos_noaddress_msg.pose.pose.position.x = 0.0;
  hedge_pos_noaddress_msg.pose.pose.position.y = 0.0;
  hedge_pos_noaddress_msg.pose.pose.position.z = 0.0;
